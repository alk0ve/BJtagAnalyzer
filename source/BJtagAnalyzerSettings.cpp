...
BJtagAnalyzerSettings::BJtagAnalyzerSettings()
	...
	mShowBitCount( false )
{
	...
	mTAPInitialStateInterface.SetTitleAndTooltip("TAP initial state", "BJTAG TAP controller initial state");
	int state_cnt;
	for (state_cnt = 0; state_cnt < NUM_TAP_STATES; ++state_cnt)
		mTAPInitialStateInterface.AddNumber(state_cnt, BJtagAnalyzerResults::GetStateDescLong(BJtagTAPState(state_cnt)), BJtagAnalyzerResults::GetStateDescLong(BJtagTAPState(state_cnt)));
...
bool BJtagAnalyzerSettings::SetSettingsFromInterfaces()
{
	...
	mTAPInitialState = BJtagTAPState(cast2Int);
...
void BJtagAnalyzerSettings::UpdateInterfacesFromSettings()
{
	mTmsChannelInterface.SetChannel(mTmsChannel);
	mTckChannelInterface.SetChannel(mTckChannel);
	mTdiChannelInterface.SetChannel(mTdiChannel);
...
void BJtagAnalyzerSettings::LoadSettings(const char* settings)
{
	...
	if (ival >= 0  &&  ival < NUM_TAP_STATES)
		mTAPInitialState = BJtagTAPState(ival);
...
const char* BJtagAnalyzerSettings::SaveSettings()
{
	SimpleArchive text_archive;
