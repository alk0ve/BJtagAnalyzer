...
BJtagAnalyzerResults::BJtagAnalyzerResults(BJtagAnalyzer* analyzer, BJtagAnalyzerSettings* settings)
:	mSettings(settings),
	mAnalyzer(analyzer)
{}

BJtagAnalyzerResults::~BJtagAnalyzerResults()
{}

void BJtagAnalyzerResults::GenerateBubbleText(U64 frame_index, Channel& channel, DisplayBase display_base)
{
	ClearResultStrings();
	...
	if (channel == mSettings->mTmsChannel)
	{
		...
		AddResultString(GetStateDescLong((BJtagTAPState) f.mType));
		AddResultString(GetStateDescShort((BJtagTAPState) f.mType));
		...
		std::set<BJtagShiftedData>::iterator sdi(mShiftedData.find(sd));
...
void BJtagAnalyzerResults::GenerateExportFile(const char* file, DisplayBase display_base, U32 export_type_user_id)
{
	...
	Frame frm;
	BJtagShiftedData sd;
	std::set<BJtagShiftedData>::iterator sdi;
	char time_str[128];
	...
	for (U64 fcnt = 0; fcnt < num_frames; fcnt++)
	{
		...
		// get the TAP state
		BJtagTAPState tap_state = (BJtagTAPState) frm.mType;
...
void BJtagAnalyzerResults::GenerateFrameTabularText(U64 frame_index, DisplayBase display_base)
{
    ClearTabularText();
    ...
    if( tms_used = true )
    {
        ...
        result_strings.push_back( GetStateDescLong((BJtagTAPState) f.mType) );
        //result_strings.push_back( GetStateDescShort((BJtagTAPState) f.mType) );
    }
...
void BJtagAnalyzerResults::GeneratePacketTabularText(U64 packet_id, DisplayBase display_base)
{
	ClearResultStrings();
	AddResultString("not supported");
}

void BJtagAnalyzerResults::GenerateTransactionTabularText(U64 transaction_id, DisplayBase display_base)
{
	ClearResultStrings();
	AddResultString("not supported");
}

void BJtagAnalyzerResults::AddShiftedData(const BJtagShiftedData& shifted_data)
{
	mShiftedData.insert(shifted_data);
}

const char* BJtagAnalyzerResults::GetStateDescLong(const BJtagTAPState mCurrTAPState)
{
...
const char* BJtagAnalyzerResults::GetStateDescShort(const BJtagTAPState mCurrTAPState)
{
	if (mCurrTAPState > UpdateIR)
		return "<undef>";
